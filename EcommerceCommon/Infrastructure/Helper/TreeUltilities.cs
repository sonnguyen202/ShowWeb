...
namespace EcommerceCommon.Infrastructure.Helper
{
    ...
    public static class TreeHelper
    {
        ...
        public static List<T> ConvertToFlatArray<T>(IEnumerable<T> trees)
            where T : class, ITreeNode<T>
        {
            List<T> treeNodeList = new List<T>();
            foreach (T rootNode in trees)
            {
                foreach (T node in Iterators.DepthFirstTraversal(rootNode))
                {
        ...
        public static T FindDescendant<T>(T searchRoot, Guid id)
            where T : class, ITreeNode<T>
        {
        ...
        /// Returns null if the node cannot be found.</summary>
        public static T FindTreeNode<T>(IEnumerable<T> trees, Guid id)
            where T : class, ITreeNode<T>
        {
        ...
        /// </example>
        public static bool HasHeirachyLoop<T>(T node)
            where T : class, ITreeNode<T>
        {
        ...
        public static T GetRootNode<T>(T node)
            where T : class, ITreeNode<T>
        {
            EnsureTreePopulated(node, "node");
        ...
        /// </summary>
        public static int GetDepth<T>(T node)
            where T : class, ITreeNode<T>
        {
            EnsureTreePopulated(node, "node");
        ...
        public static class Iterators
        {
            ...
            /// </summary>
            /// <param name="startNode">The node to start iterating from.  This will be the first node returned by the iterator.</param>
            public static List<T> FromRootToNode<T>(T node)
                where T : class, ITreeNode<T>
            {
                EnsureTreePopulated(node, "node");
            ...
            public static IEnumerable<T> DepthLeafTraversal<T>(T startNode)
                where T : class, ITreeNode<T>
            {
                EnsureTreePopulated(startNode, "node");
                ...
                else
                {
                    foreach (T child in startNode.Children)
                    {
                        foreach (T grandChild in DepthLeafTraversal(child))
                        {
                            yield return grandChild;
                        }
                    }
